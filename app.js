...
const i18n = {
  zh: {
    ...
    heroBadge: '科学算法·个性化定制',
    heroTitle1: '打造', heroTitle2: '专属你的', heroTitle3: '科学健身计划',
    ...
    f1Title: '科学计划生成', f1Desc: '根据你的数据自动生成每日训练安排，涵盖有氧、力量与柔韧性训练。',
    ...
    step2Title: '科学分析评估', step2Desc: '系统分析你的 BMI、体重差距，计算每周健康减重/增重目标。',
    ...
    shareBtn: '分享计划链接',
    shareCopied: '✅ 链接已复制！链接包含你的身体数据，请仅分享给信任的人。',
    navExercises: '动作库',
    modeAdjustNotice: (from, to) => `已根据目标体重自动调整为「${to}」模式（你选择的是「${from}」）`,
    previewBmi: 'BMI 预览',
    previewTdee: '每日消耗',
    previewCal: '目标热量',
  ...
  en: {
    ...
    heroBadge: 'Science-Powered · Personalized',
    ...
    f1Title: 'Science-Based Plan Generator', f1Desc: 'Auto-generates daily training schedules based on your data — covering cardio, strength, and flexibility.',
    ...
    step2Title: 'Scientific Analysis', step2Desc: 'Morphix analyzes your BMI, weight gap, and calculates your weekly healthy change target.',
    ...
    shareCopied: '✅ Link copied! It contains your body metrics — share only with people you trust.',
    navExercises: 'Exercise Library',
    modeAdjustNotice: (from, to) => `Adjusted to "${to}" based on your target weight (you selected "${from}")`,
...
function applyTranslations() {
  ...
  setText('navExercises', T.navExercises);
  ...
  setText('clearDataText', T.clearDataBtn);
...
// ── Calculations ──────────────────────────────────────
/** Derive effective goal from weight direction; selected goal only applies when maintaining. */
function resolveActualMode(currentWeight, targetWeight, selectedGoal) {
  if (targetWeight < currentWeight) return 'lose';
  if (targetWeight > currentWeight) return 'gain';
  return selectedGoal;
}
...
function targetCalories(tdee, mode, intensity, level, gender, bmr) {
  // Base adjustment by fitness level (documented in README), scaled by plan intensity
  const levelDeficit = { beginner: 350, intermediate: 500, advanced: 650 };
  const levelSurplus = { beginner: 200, intermediate: 300, advanced: 450 };
  const intensityFactor = { mild: 0.75, balanced: 1.0, aggressive: 1.25 };

  if (mode === 'lose') {
    const deficit = Math.round((levelDeficit[level] || 500) * (intensityFactor[intensity] || 1));
    const genderFloor = gender === 'female' ? 1400 : 1600;
    const bmrFloor = Math.round(bmr * 1.1);
    return Math.max(tdee - deficit, genderFloor, bmrFloor);
  }
  if (mode === 'gain') {
    const surplus = Math.round((levelSurplus[level] || 300) * (intensityFactor[intensity] || 1));
    return tdee + surplus;
  }
  return tdee;
}

function estimateWeeks(currentWeight, targetWeight, mode, intensity, level) {
  const diff = Math.abs(targetWeight - currentWeight);
  if (diff < 0.5 || mode === 'balance') return 0;

  const levelRate = {
    lose: { beginner: 0.3, intermediate: 0.45, advanced: 0.55 },
    gain: { beginner: 0.15, intermediate: 0.25, advanced: 0.35 }
  };
  const intensityFactor = { mild: 0.7, balanced: 1.0, aggressive: 1.25 };
  const baseRate = (levelRate[mode] && levelRate[mode][level]) || 0.45;
  const kgPerWeek = baseRate * (intensityFactor[intensity] || 1);
...
function renderResults(height, age, currentWeight, targetWeight) {
  ...
  const actualMode = resolveActualMode(currentWeight, targetWeight, goal);
  const targetCal = targetCalories(tdee, actualMode, state.intensity, level, gender, bmr);
  const proteinG = Math.round(currentWeight * getProteinMultiplier(actualMode, level));
  ...
  const calBurned = calcCaloriesBurned(currentWeight, dailyTime, actualMode, level);
  const weeks = estimateWeeks(currentWeight, targetWeight, actualMode, state.intensity, level);
  ...
  const planDays = templates[level][actualMode];
  const scheduleDays = buildSchedule(planDays, frequency, state.intensity);
  const tips = tipsDB[actualMode];

  const heightM = height / 100;
  const idealMin = (18.5 * heightM * heightM).toFixed(1);
  const idealMax = (24.9 * heightM * heightM).toFixed(1);

  const goalName = t('goalLabels')[actualMode] || actualMode;
  const levelName = t('levelLabels')[level] || level;
  const modeAdjustHtml = actualMode !== goal
    ? `<div class="mode-adjust-notice">${tf('modeAdjustNotice', t('goalLabels')[goal], goalName)}</div>`
    : '';
...
function exportPDF() {
  ...
  const cw = parseFloat(document.getElementById('currentWeight').value);
  const tw = parseFloat(document.getElementById('targetWeight').value);
  const actualMode = resolveActualMode(cw || 0, tw || 0, state.goal);
  const goalName = t('goalLabels')[actualMode] || actualMode;
...
function clearSavedData() {
  ...
  selectGender('male');
  document.getElementById('dailyTime').value = 45;
  updateTimeDisplay(45);
  selectFrequency(4);
  selectGoal('balance');
  selectIntensity('balanced');
  ...
  if (btn) {
    const label = document.getElementById('clearDataText');
    const orig = label ? label.textContent : btn.textContent;
    if (label) label.textContent = t('dataCleared');
...
function updateLivePreview() {
  ...
  if (a && a >= 15 && a <= 80) {
    ...
    const actualMode = (tw && tw >= 30 && tw <= 300)
      ? resolveActualMode(cw, tw, state.goal)
      : state.goal;
    const targetCal = targetCalories(tdee, actualMode, state.intensity, state.level, state.gender, bmr);
    const calBurned = calcCaloriesBurned(cw, state.dailyTime, actualMode, state.level);
    ...
    if (tw && tw >= 30 && tw <= 300) {
      const weeks = estimateWeeks(cw, tw, actualMode, state.intensity, state.level);
      previewCards += `
        <div class="preview-card">
